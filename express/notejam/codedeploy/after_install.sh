...
aws --region=$EC2_REGION ssm get-parameter --name "/Notejam/Database/DBPassword" --with-decryption --output text --query Parameter.Value >> /usr/share/notejam/.env
echo -n "DB_USERNAME=" >> /usr/share/notejam/.env
aws --region=$EC2_REGION ssm get-parameter --name "/Notejam/Database/DBUser" --with-decryption --output text --query Parameter.Value >> /usr/share/notejam/.env
